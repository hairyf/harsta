...
export async function compile(args: any) {
  ...
  args.clean && exec(`node ${hardhatBinRoot} clean`, { stdio: 'ignore' })
  exec(`node ${hardhatBinRoot} compile`)
  exec(`node ${hardhatBinRoot} export-abi`, { stdio: 'ignore' })
  ...
  await buildTypes()
  await buildDistributed()

  async function buildDistributed() {
    ...
    exec(`node ${tsupBinRoot} --outDir ${outdir}`, { stdio: 'ignore', cwd: generateRoot })
    exec(`node ${tscBinRoot} --outDir ${outdir}`, { cwd: generateRoot })
  ...
  async function buildContractFactories(
    ...
    for (const { outfile, input } of paths) {
      ...
      const fileRows = [
        ...
        `import { resolveAddress } from '${path.relative(dirname, resolveGenerate('./utils'))}'`,
        `import { resolveRunner } from '${path.relative(dirname, resolveGenerate('./ethers'))}'`,
